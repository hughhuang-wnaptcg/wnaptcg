...
const minIdx = levelNames.indexOf(CUSTOM_AVATAR_MIN_LEVEL)
return idx >= minIdx
}
...
setSaving(true)
const payload = { display_name: editName.trim(), avatar_url: editAvatar || null }
...
const path = `member-avatars/${member.id}/${Date.now()}.${ext}`
...
const canCustomAvatar = canUploadCustomAvatar(member.level)
...
<button onClick={() => { setEditName(member.display_name || ''); setEditAvatar(member.avatar_url || ''); setShowSettings(true) }}
style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#FAEEDA,#FFF3D0)', border: '0.5px solid #FAC775', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
...
<div style={{ marginBottom: 16 }}>
<label style={{ fontSize: 12, color: '#999', display: 'block', marginBottom: 8 }}>頭貼</label>
<div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
{editAvatar
? <img src={editAvatar} alt="" style={{ width: 54, height: 54, borderRadius: '50%', objectFit: 'cover', border: '2px solid #FAC775' }} />
: <div style={{ width: 54, height: 54, borderRadius: '50%', background: '#FAEEDA', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: '#633806', border: '2px solid #FAC775' }}>{editName?.[0]?.toUpperCase()}</div>
}
<div style={{ flex: 1 }}>
...
<div style={{ width: '100%', padding: '8px 10px', background: '#f8f5f0', border: '0.5px solid #e8e0d5', borderRadius: 8, fontSize: 12, color: '#bbb', marginBottom: 5, display: 'flex', alignItems: 'center', gap: 6 }}>
<i className="fa-solid fa-lock" style={{ fontSize: 11, color: '#D4A94A' }}></i>
<span>升至</span>
<span style={{ color: '#E24B4A', fontWeight: 600 }}>{CUSTOM_AVATAR_MIN_LEVEL}</span>
<span>後可上傳自訂頭貼</span>
</div>
)}
...
<>
<div style={{ fontSize: 11, color: '#bbb', marginBottom: 7 }}>或選擇基本頭貼</div>
<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
{defaultAvatars.map(url => (
<img key={url} src={url} alt="" onClick={() => setEditAvatar(url)}
style={{ width: 42, height: 42, borderRadius: '50%', objectFit: 'cover', cursor: 'pointer', border: editAvatar === url ? '2px solid #E07B00' : '2px solid #f0e8d0' }} />
))}
</div>
</>
)}
...
<button onClick={handleSaveSettings} disabled={saving || avatarUploading || !editName.trim()}
style={{ width: '100%', padding: 12, background: (saving || avatarUploading || !editName.trim()) ? '#f0ebe3' : 'linear-gradient(135deg,#FAEEDA,#FFF3D0)', border: '0.5px solid #FAC775', borderRadius: 10, fontSize: 14, fontWeight: 500, color: (saving || avatarUploading || !editName.trim()) ? '#ccc' : '#8B5A00', cursor: 'pointer', marginBottom: 10 }}>
{saving ? '儲存中...' : '儲存設定'}
</button>
