...
{/* ── 直播下單區入口（原本日菜單）── */}
<style>{`@keyframes liveDotHome{0%,100%{opacity:1;transform:scale(1)}50%{opacity:0.4;transform:scale(0.7)}}`}</style>
<div
onClick={() => navigate('/shop')}
style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', background: 'linear-gradient(135deg,#1a1a1a,#2d2d2d)', border: '1px solid #333', borderRadius: 12, cursor: 'pointer', marginBottom: 16, boxShadow: '0 4px 14px rgba(0,0,0,.2)' }}>
<div style={{ position: 'relative', width: 34, height: 34, borderRadius: 10, background: '#E24B4A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
<i className="fa-solid fa-video" style={{ fontSize: 14, color: '#fff' }}></i>
{menuItemCount > 0 && (
<div style={{ position: 'absolute', top: -3, right: -3, width: 8, height: 8, borderRadius: '50%', background: '#E24B4A', border: '2px solid #1a1a1a', animation: 'liveDotHome 1s ease infinite' }}></div>
)}
</div>
<div style={{ flex: 1 }}>
<div style={{ fontSize: 13, fontWeight: 700, color: '#fff' }}>直播下單區</div>
<div style={{ fontSize: 10, color: 'rgba(255,255,255,0.45)', marginTop: 1 }}>
{menuItemCount > 0 ? `${menuItemCount} 件商品上架中 →` : '敬請關注直播通知 →'}
</div>
</div>
{menuItemCount > 0 && (
<span style={{ fontSize: 10, fontWeight: 700, background: '#E24B4A', color: '#fff', padding: '2px 8px', borderRadius: 99 }}>
{menuItemCount} 件
</span>
)}
<i className="fa-solid fa-chevron-right" style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}></i>
</div>
