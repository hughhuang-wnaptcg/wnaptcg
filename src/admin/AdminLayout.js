...
const NAV = [
  ...
  { path: '/admin/cards', label: '戰績牆', icon: '🏆' },
  { path: '/admin/boss', label: 'Boss 管理', icon: '⚔️' },
          ...
          {NAV.map(n => (
            <div key={n.path} onClick={() => navigate(n.path)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 13, marginBottom: 2,
                background: pathname === n.path ? '#FCEBEB' : 'transparent',
                color: pathname === n.path ? '#A32D2D' : '#666' }}>
              <span style={{ fontSize: 16 }}>{n.icon}</span> {n.label}
            </div>
          ))}
        </div>

        <div style={{ marginTop: 'auto', padding: '12px 14px', borderTop: '0.5px solid #e5e5e5', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: '#111' }}>{member?.display_name}</div>
            <div style={{ fontSize: 10, color: '#999' }}>超級管理員</div>
          </div>
          <span style={{ fontSize: 16, cursor: 'pointer', color: '#aaa' }} onClick={signOut} title="登出">🚪</span>
        </div>
      </div>
